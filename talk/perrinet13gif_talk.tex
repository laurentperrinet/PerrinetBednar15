...
\newcommand{\Website}{http://invibe.net/LaurentPerrinet}%/Publications/Perrinet11sfn
...
\newcommand{\SubTitle}{Implications for understanding adaptation of the function of V1 with respect to the environment}%
...
This work is supported by projects ``BrainScaleS'' (EU funding, grant number FP7-269921). %
}%
\newcommand{\Conference}{Motion Clouds network meeting.}%
\newcommand{\Date}{Tuesday, 3rd of December, 2013}%
%%%%%%%%%%% faizi laTETE %%%%%%%%%%%%%%%
\usepackage{color}%
\usepackage{units}%
...
\title{\Title}%
\author{\Author $^{1,2}$ and \AuthorC $^{4}$}%, \AuthorB $^{3}$
\subtitle{\SubTitle}%
\institute{\small{$1$ - \Address \\ $2$ - \AddressBis \\ $4$ - \AddressC}}%$3$ - \AddressB \\ 
\date{\Date\   \\ %\Website \\ 
...
\note{%
	\begin{itemize}
		 \item[(hello)] \odot Hi, I am \Author\  and I work at the team "inference in Vision \& Behavior" supervised by Guillaume Masson in Marseille. My interests are in computational neuroscience, in discovering the code used to  efficiently represent images in the early visual system and the application to novel computational paradigms (sparsity, probabilities, prediction, hierarchical models). We now "compile" such models as neural networks and as parallel wafer systems in the BrainscaleS project. % 
		 \item[(today)] Today  I will present a work done during a visit in Karl Friston's team in London at the UCL: I will talk about the potential role of environment of animals as measured by edge statistics in understanding lateral connectivity in the primary visual cortex. Using these results, I will give some predictions on neurophysiological observations. This is joint work with James Bednar who is an expert in topographical models of cortical areas and David Fitzpatrick from MPI in FL% who is a leading neuroscientist, in particular in deciphering the pattern of lateral connectivity ... %This will illustrate an application of sparse coding technique to a neuroscience problem. 
		 \item[akno] Thanks to the BrainScaleS project for  funding this project.%
	\end{itemize}%
	
	Note: in these notes, some slides are missing for conciseness
	}%
}%
...
\note<2|handout:1>{%%
	 \begin{itemize}%
		 \item[(natural)] Oriented edges that constitute images of natural scenes tend to be aligned in collinear or co-circular arrangements, such as when \odot you follow the contours of these boulders: lines and smooth curves are more common than other possible arrangements of edges. See for example the work of Mariano Sigman on co-circularity in natural images (see Sigman, 2001). %
...
%	\includegraphics<1|handout:0>[width=.65\linewidth]{figure_series_11.png}%
	\includegraphics<1|handout:0>[width=.65\linewidth]{fig_architecture.png}%
	\includegraphics<2|handout:1>[width=\linewidth]{Miikkulainen05.pdf}%fig_architecture.png
\end{center}
\vfill%
%\only<1|handout:0>{[Series et al., 2002]}%
\only<1|handout:0>{[Fischer et al., 2007]}%
...
\note{%%
	This prior for smooth contours can be introduced in models:
	 \begin{itemize}
		 \item[(colin)] \odot This is a typical assumption that the role of lateral interactions is to enhance the activity of neurons which are collinear : it is the so-called \emph{association field} formalized in Field 93, %as was for instance modeled in the work from P. Series...
...
\begin{changemargin}
	...
	\includegraphics<2|handout:1>[width=\linewidth]{../database/treeshrew/00000160.jpg}
\end{changemargin}
\note{%%
...
\begin{tabular}{cc}%
\only<1|handout:0>{%
	\includegraphics[width=.468\linewidth]{testing_short_proba-edgefield_colin_serre07_distractors.png}&%
	\includegraphics[width=.468\linewidth]{edgestats_vanilla_proba-edgefield_colin_laboratory.pdf}\\%
Natural &%
Laboratory %
}%
\only<2|handout:1>{
	\includegraphics[width=.468\linewidth]{testing_short_proba-edgefield_cocir_serre07_distractors.png}&%
	\includegraphics[width=.468\linewidth]{edgestats_vanilla_proba-edgefield_cocir_laboratory.pdf}\\% 
Natural &%
Laboratory %
}%
\only<3|handout:2>{%
	\includegraphics[width=.468\linewidth]{figure2-b.pdf}&%
%	\begin{tikzpicture}
%  \begin{scope}[scale=0.45, yshift=0cm]
...
%    %----------------------------------------------------------
%
%    \tkzDefLine[perpendicular=through A, K=-.6](C,A)
%    \tkzGetPoint{a1}
...
\only<4|handout:0>{%
	\includegraphics[width=.468\linewidth]{testing_vanilla_proba-edgefield_chevrons_serre07_distractors.png}&%
	\includegraphics[width=.468\linewidth]{classifier_proba-edgefield_chevrons_laboratory.png}\\%
Natural &%
Laboratory %
}%
\end{tabular}%
...
\begin{tabular}{llcccc}
 \multicolumn{1}{c}{\bf Database 1} &\multicolumn{1}{c}{\bf Database 2} &\multicolumn{1}{c}{\bf
 2-means} &\multicolumn{1}{c}{\bf SVM 1} &\multicolumn{1}{c}{\bf SVM 2}&\multicolumn{1}{c}{\bf SVM C}
\\ \hline \\
% to update
Natural & Artificial & 98\% & 88\% & 99\% & 98\%\\
\end{tabular}
\end{table}
...
% -------------------------------------------------------------------------------------------------------------------
% -----------------------------------------------------------------------------------------------------------------------------------------------------
\section{Categorizing animals vs animals}%
\frame[label=serre07]{ %
\frametitle{Categorizing animals vs animals}%
\begin{changemargin}%
\begin{center}
\only<1|handout:0>{%
\begin{tabular}{cc}%
...
\begin{tabular}{cc}%
	\includegraphics[width=.468\linewidth]{testing_vanilla_proba-edgefield_chevrons_serre07_distractors.png}&%
	\includegraphics[width=.468\linewidth]{testing_hard_proba-edgefield_chevrons_serre07_targets.png}\\%
Natural &%
Animal %
\end{tabular}%
%	\includegraphics[width=\linewidth]{../ms/figure3.png}%
	}%
\only<4|handout:0>{%
\begin{table}[t]
%\caption{}
...
\\ \hline \\
% to update
Natural & Natural & 50\% & 50\% & 50\% & 50\%\\[1ex]
Natural (noise) & Animal (noise) & 64\% & 71\% & 77\% & 77\%\\
Natural & Animal & 65\% & 68\% & 82\% & 81\%\\
Natural & Artificial & 98\% & 88\% & 99\% & 98\%\\
\end{tabular}
\end{table}
}%
\only<5|handout:0>{%
\begin{tabular}{cc}%
	\includegraphics[width=.468\linewidth]{testing_smooth_SVM_serre07_distractors_serre07_targets_chevron_best.png}&%
	\includegraphics[width=.468\linewidth]{testing_smooth_SVM_serre07_distractors_serre07_targets_chevron_worst.png}\\%
Best &%
Worst %
\end{tabular}%
}%
\only<6|handout:2>{%
	\includegraphics[width=.8\linewidth]{serre07fig1.png}%scheme_thorpe.jpg
	}%
\only<7|handout:0>{%
\begin{tabular}{cc}%
...
\bibitem[Perrinet(2013)]{Perrinet13cerco}
L.~U. Perrinet,  D.~Fitzpatrick, and J.~Bednar
\newblock {\Title}.
\newblock \emph{\Conference}, 2013.
\newblock URL  \url{\Website/Presentations/13-07-05_CerCo}%
...
\only<+|handout:0>{%
	\includegraphics[width=.468\linewidth]{MP_512.png}&%
	\includegraphics[width=.468\linewidth]{MP_512_reconstruct.png}\\%
}%
\only<+|handout:0>{%
	\includegraphics[width=.468\linewidth]{MP_1024.png}&%
	\includegraphics[width=.468\linewidth]{MP_1024_reconstruct.png}\\%
}%
\only<+|handout:0>{%
	\includegraphics[width=.468\linewidth]{MP_2048.png}&%
	\includegraphics[width=.468\linewidth]{MP_2048_reconstruct.png}\\%
}%
Residual &%
Edges %
\end{tabular}%
