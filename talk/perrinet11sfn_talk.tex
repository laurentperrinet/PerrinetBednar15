...
\documentclass[10pt]{beamer}%,draft
...
\newcommand{\Website}{http://www.incm.cnrs-mrs.fr/LaurentPerrinet/Publications/Perrinet11sfn}%
...
This work is supported by European Union project Number FP7-269921, ``BrainScaleS''.}%
%# abstract
%
...
\newcommand{\Conference}{Society for Neuroscience 2011\\ Nanosymposium on ``Development of Motor and Sensory Systems''\\Abstract Control Number: 17671, Presentation Number: 530.04.}%
\newcommand{\Date}{Tuesday, November 15, 2011}%
%%%%%%%%%%% faizi laTETE %%%%%%%%%%%%%%%
...
\hypersetup{%
pdftitle={\Title},%
pdfauthor={\Author < \AuthorEmail > \AddressFuture - \Website},%
...
\subtitle{\SubTitle}%
\institute{$1$ - \AddressFuture \\ $2$ - \AddressBis \\ $3$ - \AddressB \\ $4$ - \AddressC}%
\date{\Date\   \\ %\Website \\ 
...
\begin{center}%
	...
	\includegraphics<2|handout:2>[width=\linewidth]{Geisler01Fig7.pdf}%
\end{center}%
\vfill%
...
\note<2|handout:1>{%%
	 \begin{itemize}%
		 \item[(natural)] Oriented edges that constitute images of natural scenes tend to be aligned in collinear or co-circular arrangements, such as when you follow the contours of these boulders: lines and smooth curves more common than other possible arrangements of edges. %
...
%\frame[label=intro2b]{%
%\begin{center}fig_architecture.png
%	\includegraphics[width=\linewidth]{Miikkulainen05.pdf}%
%\end{center}
%\vfill%
%[Choe et~al. 2004; Miikkulainen et al., 2005]%, Computational Maps in the Visual Cortex]}%
%\note{%%
%	 \begin{itemize}
...
\note{%%
	 \begin{itemize}
		 \item[(link)] However, it is not yet known whether these patterns develop as a result of visual experience, or are simply hard-wired to be appropriate for the statistics of natural scenes. To investigate this issue, we examined the properties of the visual environment of laboratory animals, to determine whether the observed connection patterns are more similar to the statistics of the rearing environment or of a natural habitat. %there is therefore a dependency between the co-linearity of the input to the primary visual cortex, the connectivity pattern of lateral interactions and our understanding of this machinery underlying early vision. This is quite general and present in the somatosensory and auditory systems.
		  \item[(natural)] Specifically, we analyzed the cooccurence statistics of edge elements in images of natural scenes, and compared them to corresponding statistics for images taken from within the rearing environment of the animals in the Bosking et al. (1997) study.  %but this connection between natural scene statistics and neurophysiology is based on some  definition of what is a natural image, which could be something like this (imagine the treeshew sitting on a tree and seeing this scene),
...
\begin{center}%
	...
	\includegraphics<2|handout:0>[width=.6\linewidth]{Geisler01Fig3A.pdf}%
\end{center}%
\vfill%
[Geisler et al., 2001, Vision Research]%
%
\note{% définition des angles = Geisler01Fig2.pdf
...
\frametitle{\insertsubsection}%
\begin{center}
	\includegraphics[width=.8\linewidth]{loggabor.png}%
\end{center}
\vfill%
...
\note{%%
	\begin{itemize}
		...
		\item[(RMSE)] This was confirmed by the reconstruction of the images from the edges which yielded an approximate RMSE of $\approx 15\%$
...
%\includegraphics<5>[width=.5\linewidth]{../figures/edgestats_big_proba-edgefield_colin_treeshew.pdf}%\includegraphics[]{../figures/classifier_proba-edgefield_cocir_treeshew.pdf}

	\only<1|handout:0>{%
$$	\arg\max_\theta p( d, \phi, \theta, \sigma | \pi_0)  $$ \\
	}%
	\only<2|handout:0>{
$$	\arg\max_\phi p( d, \phi, \theta, \sigma | \pi_0)  $$ \\
		}%
	\only<3|handout:1>{
...
%		\begin{equation}
%		\end{equation}
		\includegraphics[width=.5\linewidth]{../figures/classifier_proba-edgefield_colin_Yelmo.pdf}&%edgestats_vanilla_proba-edgefield_colin_natural.pdf}&%
		\includegraphics[width=.5\linewidth]{../figures/classifier_proba-edgefield_colin_treeshew.pdf}\\%edgestats_vanilla_proba-edgefield_colin_laboratory.pdf}\\%
	}%
	\only<2|handout:1>{
		\includegraphics[width=.5\linewidth]{../figures/classifier_proba-edgefield_cocir_holle_distractors.pdf}&%edgestats_vanilla_proba-edgefield_cocir_natural.pdf}&%
		\includegraphics[width=.5\linewidth]{../figures/classifier_proba-edgefield_cocir_treeshew.pdf}\\% edgestats_vanilla_proba-edgefield_cocir_laboratory.pdf}\\%
	}%
	\only<3|handout:2>{%
		\includegraphics[width=.5\linewidth]{../figures/classifier_proba-angle_Yelmo.pdf}&%classifier_proba-angle_holle_targets.pdf
		\includegraphics[width=.5\linewidth]{../figures/classifier_proba-angle_treeshew.pdf}\\%
	}%
	Natural &%
	Laboratory %
\end{tabular}%
...
\note<handout:2>{%
	\begin{itemize}
		 \item[(angles)]  By computing measures of the independence of the different variables, we found that the probability density function factorizes with on one side distance and scale and on the other side the 2 angles. The first component proved to be quite similar across both classes and the greater difference is seen for different angle configuration. As it can be reduced to 2 dimensions, we can plot the full probability as shown here by different contrast values assigned to all possible wedge configurations, for all possible azimuth values $\phi$ on the horizontal axis and difference of orientation $\theta$ on the vertical axis. Such a plot most strikingly shows the difference between these 2 classes.
		 by computing measures of the independence of the different variables, we found that the pdf factorizes with on one side d and $\sigma$ and on the other side the 2 angles. the first component proved to be quite similar across both classes and the greater difference is seen for different angle configuration. since it is now reduced to 2 dimensions, we can plot the full probability as shown here by different contrast values assigned to all possible wedge configurations, changing $\phi$ on the horizontal axis and $\theta$ on the vertical. such a plot most strikingly the difference between these 2 classes.
	\end{itemize}
...
\frametitle{\insertsubsection}%
\begin{center}
\includegraphics[width=.7\linewidth]{../figures/classifier_KL_holle_distractors_holle_targets.pdf}%edgestats_vanilla_KL_natural_laboratory.pdf}
\end{center}
\note{%%
	\begin{itemize}
		 \item[(KL-classifier)] ... we did that by using a simple classifier (a 2-means classifier using the KL distance between the histogram of one image  to the average histogram to each class), so that one gets a simple, translation, orientation and scale invariant classifier which can efficiently differentiate between one natural image and a lab image. it comes as a big surprise as this is only based on some local characteristic, but it sufficient to get good classification. this gives also a quantitative method (measuring the area under the ROC curve) to rate different methods and databases. The result as computed by the Area Under the Curve is of 99.3\% accuracy.
...
\only<1|handout:0>{%
\begin{tabular}{cc}%
	\includegraphics[width=.5\linewidth]{edgestats_vanilla_natural_frame570.png}&\includegraphics[width=.5\linewidth]{classifier_treeshew_00000230.png}\\%
}\only<2|handout:1>{%
	$$	 p( d, \phi, \theta, \sigma | \pi_0) \approx  p( d, \sigma  | \pi_0) p( \theta,\phi | \pi_0)  $$ \\
\begin{tabular}{cc}%
		\includegraphics[width=.5\linewidth]{../figures/classifier_proba-angle_Yelmo.pdf}&%classifier_proba-angle_holle_targets.pdf
